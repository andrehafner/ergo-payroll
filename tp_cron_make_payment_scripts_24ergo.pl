...
my $end_filename = "_24erg_tokens.py";

#this mysql call gets the company info as a base 
my $sql_access_main = "select company_wallet_deposit_id, company_wallet_id, company_seed_name from employer where company_wallet_deposit_id!='' and company_wallet_id!='' and company_active='yes' and company_24_frequency='yes';";
...
node_url: str = "http://213.239.193.208:9053/" # MainNet or TestNet
...
$sql = "select e_company_wallet_id, employee_token_id from employee where company_wallet_id='@array_access_main[1]' and employee_token_id='erg' and employee_pay_frequency='24' and employee_active_status='yes' order by id;";
...
$sql = "select employee_token_count, employee_token_id, employee_usd_amount from employee where company_wallet_id='@array_access_main[1]' and employee_token_id='erg' and employee_pay_frequency='24' and employee_active_status='yes' order by id;";
...
my $sql_price = "select @array[1] from pricedata;";
...
#if (@array[$int+3] eq '' && @array[$int] eq '' && @array[$int+2] eq 'erg' && @array[$int+1] ne ''){
#my $price = $array[$int+1] / $array_price[0];
...
if (@array[$int+3] ne '' && @array[$int] ne '' && @array[$int+5] ne '' && @array[$int+1] eq 'erg'){
print $fh $array[$int];
print $fh ",";
}

if (@array[$int+3] ne '' && @array[$int] ne '' && @array[$int+5] eq '' && @array[$int+1] eq 'erg'){
print $fh $array[$int];
print $fh ",";
}


if (@array[$int+3] eq '' && @array[$int] ne '' && @array[$int+5] ne '' && @array[$int+1] eq 'erg'){
print $fh $array[$int];
print $fh ",";
}


if (@array[$int+3] ne '' && @array[$int+2] ne '' && @array[$int+5] ne '' && @array[$int+1] eq 'erg'){
my $price = $array[$int+2] / $array_price[0];
$price = sprintf("%.4f", $price);
print $fh "$price";
}

if (@array[$int+3] ne '' && @array[$int+2] ne '' && @array[$int+5] eq '' && @array[$int+1] eq 'erg'){
my $price = $array[$int+2] / $array_price[0];
$price = sprintf("%.4f", $price);
print $fh "$price";
}
...
if (@array[$int+3] eq '' && @array[$int+2] ne '' && @array[$int+5] ne '' && @array[$int+1] eq 'erg'){
my $price = $array[$int+2] / $array_price[0];
$price = sprintf("%.4f", $price);
print $fh "$price";
}
...
if (@array[$int+3] eq '' && @array[$int] ne ''  && @array[$int+5] eq '' && @array[$int+1] eq 'erg'){
print $fh $array[$int];
print $fh "";
}

if (@array[$int+3] eq '' && @array[$int+2] ne ''  && @array[$int+5] eq '' && @array[$int+1] eq 'erg'){
my $price = $array[$int+2] / $array_price[0];
$price = sprintf("%.4f", $price);
print $fh "$price";
}
...
output_main = helper_functions.simple_send(ergo=ergo, amount=amount, wallet_mnemonic=wallet_mnemonic, receiver_addresses=receiver_addresses)


with open("erg_" +str(c_wallet_address), \'w\') as f:
