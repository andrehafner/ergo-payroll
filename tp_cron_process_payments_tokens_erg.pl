...
my @files = </usr/lib/payments_payroll/*erg_tokens.py>;
...
foreach (@files){
  ...
  if (@files[$int] =~ m/_12erg/){
    $pay_frequency = '12';
  }
    #look at filename for the pay cycle
    elsif (@files[$int] =~ m/_24erg/){
      $pay_frequency = '24';
    }
    #look at filename for the pay cycle
    elsif (@files[$int] =~ m/_52erg/){
      $pay_frequency = '52';
    }
  ...
  $tessst = "/usr/lib/cgi-bin/tosipayroll/erg_$tessst";
  ...
  if (@files[$int] ne ''){
    ...
    my $sql = "update employee set paid_this_sgement='yes', employee_pay_history='$tx_address' where company_wallet_id='@files[$int]' and employee_pay_frequency='$pay_frequency' and employee_active_status='yes' and employee_token_id='erg';";
